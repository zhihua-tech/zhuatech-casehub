...
const title='知华企业案件协作', subtitle='案件受理、任务调查、证据管理与结案复核';
const token=ref(sessionStorage.getItem('casehub-session')||''),user=ref(null),catalog=ref(null),dashboard=ref(null);
...
async function request(path,{method='GET',body,key,raw=false}={}){
 ...
 if(!r.ok){let m='请求失败';try{m=(await r.json()).message||m;}catch{}if(r.status===401){token.value='';user.value=null;sessionStorage.removeItem('casehub-session');}throw new Error(m);}
...
async function login(){await guarded(async()=>{const r=await request('/auth/login',{method:'POST',body:{username:username.value,password:password.value}});token.value=r.token;user.value=r.user;sessionStorage.setItem('casehub-session',r.token);password.value='';await load();});}
async function logout(){await guarded(async()=>{await request('/auth/logout',{method:'POST',body:{}});token.value='';user.value=null;sessionStorage.removeItem('casehub-session');});}
...
function canUpload(row){return user.value.role!=='VIEWER'&&!(row.module==='cases'&&row.state==='CLOSED')&&!(row.module==='evidence'&&(row.state!=='REGISTERED'||row.data.currentHolder&&row.data.currentHolder!==user.value.username));}
...
async function exportCsv(){await guarded(async()=>{const blob=await request('/export/'+section.value,{raw:true});download(blob,'casehub-'+section.value+'.csv');});}
...
<template>
 ...
 <div v-else class="app-shell" :style="{'--accent':catalog?.accent}">
  <aside class="sidebar">
   <div class="brand"><span class="brand-mark">知</span><div><strong>{{title.replace('知华','')}}</strong><small>ZHUATECH · CASEHUB</small></div></div>
  ...
  <div v-if="selected&&module" class="drawer-backdrop" @click.self="selected=null">
   <section class="drawer" role="dialog" aria-modal="true" aria-label="业务详情">
    <div class="drawer-header"><div><small>{{module.label}} / 详情</small><h2>{{selected.code}}</h2></div><button aria-label="关闭详情" @click="selected=null">✕</button></div>
    <div class="drawer-content">
     <div class="detail-status"><span class="status" :data-state="selected.state">{{format(selected.state)}}</span><span class="muted">版本 {{selected.version}}</span></div>
     <div class="detail-actions"><button v-if="module.editable&&selected.state===module.initial&&user.role!=='VIEWER'" :disabled="busy" @click="openForm('edit',selected)">编辑资料</button><button v-for="action in module.actions.filter(a=>canAct(user,selected,a))" :key="action.key" class="primary" :disabled="busy" @click="openForm('action',selected,action)">{{action.label}}</button></div>
     <p class="muted detail-help">仅展示当前账号与单据状态允许的操作。提交后将执行关联校验并保存审计记录。</p>
     <h3>业务数据</h3><dl class="data-grid"><template v-for="(value,key) in selected.data" :key="key"><dt>{{label(key)}}</dt><dd><pre v-if="typeof value==='object'">{{format(value)}}</pre><template v-else>{{format(value)}}</template></dd></template></dl>
     <h3>附件与凭证</h3><div v-for="file in attachments" :key="file.id" class="file-row"><button class="text-button" @click="downloadFile(file)">{{file.filename}}</button><small>{{Math.ceil(file.size_bytes/1024)}} KB · SHA-256 {{file.digest.slice(0,12)}}…</small></div>
     <label v-if="canUpload(selected)" class="upload-control">＋ 上传附件<input type="file" accept=".pdf,.png,.jpg,.jpeg,.txt,.csv,.xlsx,.docx" @change="upload" :disabled="busy"></label>
     <p class="muted">单个文件不超过 2 MB；下载需要登录。请勿上传无关个人信息。</p>
     <h3>操作轨迹</h3><div v-for="(event,index) in history" :key="index" class="history-row"><strong>{{event.actor}} · {{event.action}}</strong><p>{{event.remark}}</p><time>{{prettyTime(event.created_at)}}</time><details><summary>查看变更快照</summary><pre>{{event.after_json}}</pre></details></div>
    </div>
   </section>
  </div>
