...
test('evidence custody actions follow current holder',()=>{const row={module:'evidence',creator:'operator-id',state:'REGISTERED',data:{currentHolder:'operator'}};const transfer={key:'transfer',from:['REGISTERED'],role:'OPERATOR',separate:false};const seal={key:'seal',from:['REGISTERED'],role:'REVIEWER',separate:true};assert.equal(canAct({id:'reviewer-id',username:'reviewer',role:'REVIEWER'},row,transfer),false);assert.equal(canAct({id:'operator-id',username:'operator',role:'OPERATOR'},row,transfer),true);assert.equal(canAct({id:'reviewer-id',username:'reviewer',role:'REVIEWER'},row,seal),false);assert.equal(canAct({id:'admin-id',username:'admin',role:'ADMIN'},row,transfer),true);});
