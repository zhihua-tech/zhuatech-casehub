...
| 证据封存 | 先上传文件，再由另一位审核人员核验封存；文件清单计算摘要，封存后不能追加。 |
| 结案申请 | 全部任务完成且至少一份证据已封存才允许提交处理结论。 |
| 复核归档 | 审核人员独立批准或退回；批准后产生不可修改的结案档案。 |
...
业务端提供案件/任务/证据/结案档案与 H5 适配；管理端提供用户、权限和审计。附件限制大小及类型，下载必须登录。请勿用演示模式存放真实敏感案件资料；生产需要单独完成保密分级、存储加密、留存期限及法务合规评估。
